...
if __name__ == '__main__':
    model = torch.load("./weight/prebest_wjp_3convnextPlusv1+0.003OneCycleLR+dp=0.6.pth")
    ...
    print("使用的权重文件是:prebest_wjp_3convnextPlusv1+0.003OneCycleLR+dp=0.6.pth")
    for i in range(7):
        print(f"Acuracy of {classes[i]:5s}: {100 * class_correct[i] / class_total[i]:2.0f}%")
    ...
    plt.figure(figsize=(7, 5))
    sns.heatmap(cm, annot=labels, fmt='s', xticklabels=classes, yticklabels=classes, linewidths=0.1)
    plt.savefig('./preoutcome/acc_prebest_wjp_3convnextPlusv1+0.003OneCycleLR+dp=0.6.png')
    plt.show()
