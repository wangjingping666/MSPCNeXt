...
class InceptionDWConv2d_a(nn.Module):
    """ Inception depthweise convolution
    """
    def __init__(self, in_channels, square_kernel_size=7, band_kernel_size=11, branch_ratio=0.3):
        super().__init__()
        gc = int(in_channels * branch_ratio)  # channel numbers of a convolution branch
        self.dwconv_hw = nn.Conv2d(gc, gc, 3, padding=3 // 2)
        self.dwconv_w = nn.Conv2d(gc, gc, square_kernel_size, padding=square_kernel_size // 2)
...
class InceptionDWConv2d_b(nn.Module):
    """ Inception depthweise convolution
    """
    def __init__(self, in_channels, square_kernel_size=7, band_kernel_size=11, branch_ratio=0.25):
        super().__init__()
        gc = int(in_channels * branch_ratio)  # channel numbers of a convolution branch
        self.dwconv_3 = nn.Conv2d(gc, gc, 3, padding=3 // 2, groups=gc)
        self.dwconv_hw = nn.Conv2d(gc, gc, square_kernel_size, padding=square_kernel_size // 2, groups=gc)
        self.dwconv_w = nn.Conv2d(gc, gc, kernel_size=(band_kernel_size, band_kernel_size), padding=(band_kernel_size // 2, band_kernel_size // 2),
                                  groups=gc)
        self.dwconv_h = nn.Conv2d(gc, gc, kernel_size=(band_kernel_size, band_kernel_size), padding=(band_kernel_size // 2, band_kernel_size // 2),
                                  groups=gc)
        self.split_indexes = (in_channels - 3 * gc, gc, gc, gc)

    def forward(self, x):
        x_id, x_hw, x_h ,x_w= torch.split(x, self.split_indexes, dim=1)
        x_hw = self.dwconv_hw(x_hw)
        x_h = self.dwconv_w(x_h)
        x_h = self.dwconv_h(x_h)
        x_w = self.dwconv_3(x_w)
        x = torch.cat((x_id,x_hw,x_h,x_w),dim=1)
        return x
class InceptionDWConv2d_c(nn.Module):
    """ Inception depthweise convolution
    """

    def __init__(self, in_channels, square_kernel_size=7, band_kernel_size=11, branch_ratio=0.5):
        super().__init__()

        gc = int(in_channels * branch_ratio)  # channel numbers of a convolution branch
        self.dwconv_1 = nn.Conv2d(in_channels, gc,kernel_size=(1,1))
        self.dwconv_w = nn.Conv2d(gc, gc, kernel_size=(1, band_kernel_size), padding=(0, band_kernel_size // 2),
                                  groups=gc)
        self.dwconv_h = nn.Conv2d(gc, gc, kernel_size=(band_kernel_size, 1), padding=(band_kernel_size // 2, 0),
                                  groups=gc)
        self.split_indexes = (in_channels - 1 * gc, gc)
    ...
    def forward(self, x):
        ...
        return x
class InceptionDWConv2d_d(nn.Module):
    """ Inception depthweise convolution
    """

    def __init__(self, in_channels, square_kernel_size=7, band_kernel_size=7, branch_ratio=0.5):
        super().__init__()
...
class MetaNeXt(nn.Module):
    ...
    def __init__(
        ...
        for i in range(num_stage):
            out_chs = dims[i]
            if i==0:
                token_mixers = InceptionDWConv2d_a
            elif i==1:
                token_mixers = InceptionDWConv2d_b
            elif i==2:
                token_mixers = InceptionDWConv2d_c
            elif i==3:
                token_mixers = InceptionDWConv2d_d
...
@register_model
def inceptionnext_tiny_gai(pretrained=False, **kwargs):
...
@register_model
def inceptionnext_small_gai(pretrained=False, **kwargs):
...
@register_model
def inceptionnext_base_gai(pretrained=False, **kwargs):
