...
#         #
#         self.branch1x1 = BasicConv2d(dim, 64, kernel_size=1,groups=64)
#
...
#         self.branch3x3dbl_5 = BasicConv2d(64, 64, kernel_size=3, padding=1)
#         # self.branch3x3dbl_5 = BasicConv2d(dim, 96, kernel_size=3, padding=1)
#         self.branch3x3dbl_6 = BasicConv2d(64, 64, kernel_size=3, padding=1)
#         self.branch3x3dbl_7 = BasicConv2d(64, 64, kernel_size=3, padding=1)
#
#         self.branch_pool = BasicConv2d(dim, pool_features, kernel_size=1)
#         # self.norm = nn.LayerNorm(dim, eps=1e-6)
#         # self.pwconv1 = nn.Linear(96+96+96+pool_features, 4 * dim)
#         self.pwconv1 = nn.Linear(64*4+pool_features, 4 * dim)
#         self.act = nn.GELU()
...
class Block_1(nn.Module):
    def __init__(self, dim,pool_features,drop_path=0.6, layer_scale_init_value=1e-6):
        ...
        # self.branch3x3_1 = nn.Conv2d(dim, 32, kernel_size=1)
        self.branch3x3_2 = nn.Conv2d(dim, 96, kernel_size=3, padding=1,groups=96)
        self.bn2 = BN(96, eps=0.00001)
        # self.branch3x3_2 = BasicConv2d(dim, 96, kernel_size=3, padding=1)

        # self.branch3x3dbl_1 = nn.Conv2d(dim, 32, kernel_size=1)
        self.branch3x3dbl_2 = nn.Conv2d(dim, 96, kernel_size=3, padding=1,groups=96)
        # self.branch3x3dbl_2 = BasicConv2d(dim, 96, kernel_size=3, padding=1)
        self.branch3x3dbl_3 = nn.Conv2d(96, 96, kernel_size=3, padding=1,groups=96)
        self.bn3 = BN(96, eps=0.00001)

        # self.branch3x3dbl_4 = nn.Conv2d(dim, 32, kernel_size=1)
        self.branch3x3dbl_5 = nn.Conv2d(dim, 96, kernel_size=3, padding=1,groups=96)
        # self.branch3x3dbl_5 = BasicConv2d(dim, 96, kernel_size=3, padding=1)
